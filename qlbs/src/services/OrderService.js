...
export const deleteMany = async (id, access_token,) => {
  const res = await axiosJWT.post(`${process.env.REACT_APP_API_URL}/order/delete-many`,  {
      headers: {
          token: `Bearer ${access_token}`,
      }
  })
...
export const approveOrder = async (access_token) => {
  const res = await axiosJWT.get(`${process.env.REACT_APP_API_URL}/order/get-all-order`, {
      headers: {
          token: `Bearer ${access_token}`,
      }
